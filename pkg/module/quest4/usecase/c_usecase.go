...
func (service *quest4UseCase) QuestC(country string) (*model.Quest4CResponse, error) {
	...
	query := fmt.Sprintf("SELECT [ID],[Country],[City],[Year],[Pm25],[Latitude],[Longitude],[Population],[Wbinc16_text],[Region],[Conc_pm25],[Color_pm25] FROM dbo.AirPollutionPM25 WHERE Country='%s' ORDER BY Year", country)
